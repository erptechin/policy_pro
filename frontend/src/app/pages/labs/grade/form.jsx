      ...
      if (customer) {
        try {
          ...
          const response = await getCustomData({
            url: `policy_pro.api.custom.get_customer_address?customer_name=${customer}`
          });
  ...
  useEffect(() => {
    const fetchChildItems = async () => {
      if (recipeCode) {
        try {
          const response = await getCustomData({
            url: `policy_pro.api.custom.get_child_items?parent=Recipe&child=Recipe Items&parent_name=${recipeCode}&custom_customer=${customer}`
          });
