...
export default function CODDocuments({ id, data, setValue, refetchData, filterBy = "lead", customerId = null }) {
  ...
  const filterField = filterBy === "customer" ? "customer" : "lead";
  const filterValue = filterBy === "customer" ? (customerId || data?.name || id) : id;
  const customerValue = filterBy === "customer" ? (customerId || data?.name || id) : (data?.customer || "");

  const [newCODDocument, setNewCODDocument] = useState({
    lead: filterBy === "lead" ? id : data?.lead_name || "",
  ...
  // Fetch COD Management records filtered by lead or customer
  const [search, setSearch] = useState({
    ...
    filters: filterValue ? JSON.stringify([[filterField, "=", filterValue], ["type", "!=", "New"]]) : null
  });
  ...
  useEffect(() => {
    if (filterValue) {
      const carProfileFilters = [["customer", "=", filterValue], ["status", "=", "Active"]]

      getListData({
        doctype: "Car Profile",
        fields: JSON.stringify(["name", "car_make", "car_model", "car_year"]),
        filters: JSON.stringify(carProfileFilters),
        page_length: 100
      }).then((res) => {
        if (res?.data) {
  ...
  // Update search when filter value changes
  useEffect(() => {
    if (filterValue) {
      setSearch(prev => ({
        ...prev,
        filters: JSON.stringify([[filterField, "=", filterValue], ["type", "!=", "New"]])
      }));
    }
  ...
  const handleAddCOD = async () => {
    ...
    // If filtering by customer and lead is not set, try to get it from data
    const codData = { ...newCODDocument };
    if (filterBy === "customer" && !codData.lead && data?.lead) {
      codData.lead = data.lead;
    }

    setIsAddingCOD(true);
    try {
      const result = await createCODManagement(codData);
  ...
  const handleEditClick = (codId) => {
    ...
    if (codDoc) {
      setNewCODDocument({
        lead: codDoc.lead || (filterBy === "lead" ? id : "") || "",
        customer: codDoc.customer || customerValue || "",
  ...
  useEffect(() => {
    const customer = filterBy === "customer" ? (customerId || data?.name || id) : (data?.customer || "");
    if (customer) {
  ...
  }, [data?.customer, customerId, filterBy, id, data?.name]);
            ...
            {isLoadingCOD ? (
              ...
              <div className="flex flex-col items-center justify-center py-12">
                <p className="text-gray-500 dark:text-dark-300">
                  No Endorsement yet. Click &quot;Add New Record&quot; to add one.
                </p>
              </div>
            )}
          </div>
      ...
      <ConfirmModal
        ...
        messages={{
          pending: {
            ...
            description: "Are you sure you want to delete this Endorsement? This action cannot be undone.",
            actionText: "Delete"
          }
        }}
      />
