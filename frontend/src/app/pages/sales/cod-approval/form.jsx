            ...
            <div className="col-span-12 space-y-4 sm:space-y-5 lg:col-span-4 lg:space-y-6">
              <Card className="space-y-5 p-4 sm:px-5">
                {data?.file_attachment && (
                  <button type="button" className="w-full h-full object-cover bg-primary-500 text-white p-2 rounded-md cursor-pointer" onClick={() => window.open(`${JWT_HOST_API}${data?.file_attachment}`, '_blank')}>VIEW FILE</button>
                )}
                <DynamicForms
                  infos={info}
                  fields={subFields}
