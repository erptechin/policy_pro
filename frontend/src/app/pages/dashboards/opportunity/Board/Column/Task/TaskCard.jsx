...
export const TaskCard = forwardRef(({ lists, data, columnData, ...rest }, ref) => {
  const { id, color } = data;
  ...
  const { instanceId, registerCard, doctype } = useBoardContext();
  ...
  useEffect(() => {
    ...
    return combine(
      draggable({
        ...
        onDrop: (data) => {
          if (data?.source?.data?.cardId) {
            const currentColumnId = data?.location?.current?.dropTargets?.length
            const statusId = data.location.current.dropTargets[currentColumnId - 1].data.columnId
            const status = lists.find(item => item.id === statusId)
            const id = data.source.data.cardId
            mutationUpd.mutate({ doctype, body: { status: status.name, id } })
          }
      ...
      {draggableState.type === "preview" &&
        createPortal(
          <Box
            ...
            className="box-border rounded-lg bg-gray-300 backdrop-blur-xs dark:bg-dark-600"
          >
            <CardPrimitive data={data} />
          </Box>,
