...
export function SelectedRowsActions({ table }) {
  ...
  const confirmMessages = {
    pending: {
      description:
        `Are you sure you want to delete those ${doctype}? Once deleted, it cannot be restored.`,
    },
    success: {
  ...
  const handleDeleteRows = () => {
    if (selectedRows.length > 0) {
      const ids = selectedRows.map(field => field.original.id);
      setDeleteLoading(true);
      mutation.mutate({ doctype, ids: ids })
    }
  };
