...
const SearchSelectDynamic = forwardRef(({ control, option, onChange, value, name, error, label, placeholder, isAddNew, rootItem, readOnly }, ref) => {
  ...
  useEffect(() => {
    if (doctype) {
      getListData({
        ...
        if (res?.data) {
          setLists(res.data.map(item => ({ label: item.name, value: item.name })))
        }
      })
    }
  }, [doctype]);
...
SearchSelectDynamic.displayName = "SearchSelectDynamic";

export { SearchSelectDynamic };
